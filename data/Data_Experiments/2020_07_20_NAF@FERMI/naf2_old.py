...
class ReplayBuffer:
    ...
    def save_to_pkl(self, name, directory):
        buffer_data = dict(obs1=self.obs1_buf,
                           obs2=self.obs2_buf,
                           acts=self.acts_buf,
                           rews=self.rews_buf,
                           done=self.done_buf)
...
class NAF(object):
    def __init__(self, env,
                 max_steps, max_episodes, training_info=dict(), pretune=None, prio_info=dict(),
                 noise_info=dict(), save_frequency=500, directory=None, is_continued=False,
                 clipped_double_q=2, q_smoothing=0.01, warm_up_steps=None, training_stop=None, **nafnet_kwargs):
        '''
        ...
        self.q_main_model_1 = QModel(obs_box=self.obs_box, act_box=self.action_box, learning_rate=learning_rate,
                                     ...
                                     **nafnet_kwargs)
        self.q_main_model_1.create_buffers(per_flag=self.per_flag, prio_info=prio_info)
        ...
        # Set same initial values in all networks
        self.q_target_model_1 = QModel(obs_box=self.obs_box, act_box=self.action_box, name='q_target_model_1',
                                       **nafnet_kwargs)
        ...
        if self.clipped_double_q:
            self.q_main_model_2 = QModel(obs_box=self.obs_box, act_box=self.action_box, learning_rate=learning_rate,
                                         ...
                                         clipped_double_q=self.clipped_double_q,
                                         **nafnet_kwargs)
            self.q_main_model_2.create_buffers(per_flag=self.per_flag, prio_info=prio_info)
    ...
    def run(self, is_train=True):
        ...
        for index in tqdm(range(0, self.max_episodes)):
            ...
            for t in range(0, self.max_steps):
                ...
                if t % initial_max == 0 and self.q_main_model_1.replay_buffer.size <= self.warm_up_steps:
                    o = self.env.reset()
    ...
    def train_model(self, model):
        ...
        loss = model.train_model_1(**self.training_info)[-1]
    ...
    def update_q(self, model):
        ...
        if self.counter % self.save_frequency == 0:
            ...
            self.q_main_model_1.replay_buffer.save_to_pkl(name='test.pkl', directory=self.directory)
        if model == self.q_main_model_1:
