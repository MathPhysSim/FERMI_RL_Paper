...
\begin{align}
     \min_\theta\left( (r(s_t,a_t)+\gamma V_{\theta\prime}(s_{t+1}) - (1+\gamma) Q_\theta(s_t,a_t)\right)^2
\end{align}
$\theta\prime$ denotes the weights of a previous iteration. The optimal policy is given by:
\begin{align}
    \pi^*(s)=(\delta(a)-\arg\!\max_a Q_\theta(s,a)).
\end{align}
The modification, which is used in these tests is a twin network ($\tilde\theta$), where only one is used to obtain the policy, while the other is used for the update rule
