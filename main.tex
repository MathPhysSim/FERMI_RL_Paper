...
Assume states $s$ the set of all states $\states$ and actions $a$ all actions $\actions$ and rewards $r$ and the set of all rewards $\mathcal{R}$ an initial state distribution $\initstate$.
Goal of reinforcement learning is to find a $\policy\mapsto a$, which is the solution of:
\begin{align}
\max J (\policy)  =  \mathbb E_{\traj \thicksim p_{\policy}}\left[\sum_{t=0}^{\horizon}\discount^t r(\bs_t,\ba_t)\right],
\end{align}
...
\begin{align}
 \bellman^*\vec Q_\theta(s_t,a_t) := r(s_t,a_t)+\gamma\max_a\left( Q_\theta(s_{t+1},a_t) - Q_\theta(s_t,a_t)\right).
\end{align}
