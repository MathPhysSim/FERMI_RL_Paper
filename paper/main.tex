...
\usepackage[allcolors=blue,colorlinks=true,citecolor=blue,linkcolor=blue,urlcolor=blue]{hyperref}% add hypertext capabilities
...
%%height=10in,a5paper,hmargin={3cm,0.8in},
%]{geometry}
% \usepackage{wasysym} % already loaded on line 42
...
\begin{document}
	...
	\begin{abstract}
		Reinforcement learning provides a viable framework for autonomous accelerator control. This paper demonstrates the operational applicability of this approach to accelerator physics problems. Despite the success of model-free reinforcement learning in several domains, sample efficiency remains a limiting factor, a challenge that model-based methods can mitigate.
		We compare a model-based DYNA-style approach with a model-free deep Q-learning approach for the intensity optimisation of the FERMI FEL system. We find that the model-based approach demonstrates higher representational power and sample efficiency, while the asymptotic performance of the model-free method is slightly superior. The model-based algorithm uses an uncertainty-aware ensemble model, and the model-free algorithm is based on a tailored normalised advantage function. In both cases, the algorithms were designed for increased noise robustness as required in accelerator control problems. Code is released at \url{https://github.com/MathPhysSim/FERMI_RL_Paper}.
	...
	In particle accelerator operation, one main goal is to provide stable and reproducible performance. Achieving this requires addressing several control problems simultaneously \cite{Kain2020,Scheinker2018}. Particularly when a priori physics modeling is infeasible, operators typically employ optimisation techniques such as derivative-free optimisers (DFOs) \cite{Huang2013,Bruchon2017,Scheinker2020,Hirlaender2019,Welsch2015,Albright2019} or model-based optimisations such as Gaussian Processes \cite{Hanuka2020,Roussel2020} to restore or maintain performance.\\
	...
	\begin{itemize}
		...
		\item It retains structural knowledge across episodes, unlike DFOs that restart from scratch.
		\item Already existing data can be used.
		\item Adaptation to non-stationary systems is possible.
		\item The underlying structure of the problem might be deduced.
	\end{itemize}
	A critical challenge in applying RL to real-world systems is the number of iterations needed to train a controller---the sample efficiency---since RL methods are known to be data-intensive \cite{Sutton2018,DulacArnold2019}. A second critical aspect is the robustness of training in a noisy, data-limited environment.\\
...
%	We show that our algorithms require significantly less than 1000 training sets, after which they can maximise the FERMI FEL's output intensity to $>$ that 95\% of the maximum within only $\sim 3$ to $\sim 5$ steps depending on which algorithm is used.
	Leveraging model ensembles and Bayesian neural networks, our algorithms exhibit high sample-efficiency (learning from $<1000$ data sets), low model-bias, and are extremely fast, maximising the FERMI FEL's output intensity to greater than 95\% of the maximum within only 3--5 steps.
	 Thus, we demonstrate the operational applicability of purely model-based reinforcement learning to an accelerator control problem.\\
	...
	\begin{itemize}
		\item Description of the problem setup at FERMI
		\item Overview of RL
	...
	\section{The setup of the studied problem}
	\subsection{The Physical Setup}
	In a seeded free-electron laser, among the most critical parameters are the temporal and transverse overlap of the electron and laser beams in the magnetic section called the modulator undulator.\\
	...
	In recent years, various approaches have been studied to investigate their applicability in machine tuning. \\
	A free-electron laser is a fourth-generation light source where the lasing medium consists of very-high-speed electrons moving freely through a magnetic structure. By using an external seeding source, the FERMI FEL has several advantages relative to standard FEL approaches in terms of increased stability in pulse and photon energy, reduced size, improved longitudinal coherence, more control over the pulse structure, and improved temporal synchronisation with external timing systems.\\
	The external optical laser signal interacts with the relativistic electron beam in the undulator, introducing an energy modulation that aids in the FEL process. The dispersive section converts this energy modulation into a density modulation, and finally, the density-modulated beam's radiation is amplified in the radiator section.\\
	...
	\begin{figure}
		...
		\caption{A schematic view on the setup of the FERMI FEL.}
	...
	A schematic overview of the setup is provided in \cref{fig:schematic_FEL}.
	Two mirrors, TT1 and TT2, are used to control the trajectory of the laser by tilting and inclining them, yielding a total of four degrees of freedom (DOF). In turn, the laser overlaps with the electron beam between the two removable screens, CCD1 and CCD2. Lastly, the monitor measures the intensity, $I_0$.\\
	...
	During the training phase, exploration and the training of the function approximator take place.
	...
	Most commonly, two main categories of deep RL algorithms are distinguished: model-free RL (MFRL) and model-based RL (MBRL). MFRL algorithms learn directly from the interaction with a system, while MBRL algorithms learn a dynamics model from these interactions first. Despite the success of MFRL in several domains such as robotics and video games \cite{Heess2017, Schulman2017,Silver2014,Lillicrap2015,OpenAI2018}, their deployment has been largely confined to simulated environments due to their low sample efficiency. However, putting constraints on the algorithms, as discussed in \cref{s:Model-free reinforcement learning}, partly cures the sample inefficiency, turning them into practical tools for real-world problems.\\
	On the other hand, MBRL shows much better sample efficiency. The challenge for these methods is to learn an accurate model, which turns out to be rather hard and leads to the so-called model-bias problem \cite{Deisenroth2011}. In \cref{ss:Uncertainty aware DYNA-style reinforcement learning} we discuss attempts to capture the model uncertainty to alleviate this obstacle by making use of model ensembles and Bayesian neural networks resulting in algorithms, exhibiting a high sample efficiency and a low model-bias.\\
	...
	The goal of reinforcement learning is to find a $\policy^*: \bs\mapsto \ba$, which is the solution of:
	\begin{align}
	...
	In the modern field of RL, a key distinction is whether the policy $\policy(\ba)\approx \policy_\phi(\bs)$ or the state-action value function $Q(\bs,\ba) \approx Q_\theta(\bs,\ba)$ is approximated using a high-capacity function approximator, such as a deep neural network.\\
	In the first case $\policy$ is optimised directly and one speaks about policy gradient methods \cite{Sutton2018, Williams1992,Baxter2011,pmlr-v28-levine13, Schulman2015,Schulman2017}. On-policy training requires fresh interaction data for every update, limiting sample efficiency. Hence, we exclude them as candidates for direct online training but make use of their characteristics in MBRL (\cref{ss:critical_design}).\\
	In the second case, we speak about Q-learning, which belongs to approximate dynamic programming. One advantage is the possible use of previously stored data in an \emph{off-policy} fashion, which generally needs fewer data to be trained than \emph{on-policy} methods. A combination of both is also used, known as actor-critic methods \cite{Szepesvari2010,Lillicrap2015,Silver2014}.\\
	As already adduced, MFRL algorithms learn directly from interactions with the system. An exhaustive overview can be found, e.g. in \cite{Sutton2018,Levine2020}. In the following, we discuss the normalised advantage function (\emph{NAF}) algorithm \cite{Gu2016} in some detail, as it has good characteristics for accelerator control problems. It has high sample efficiency, and its representational power is sufficient for most common cases\cite{Kain2020,Hirlaender2020a}. Modifications to increase the stability are subject of \cref{ss:Normalized advantage function}.
	...
	\end{align}
	The state-action-value function - short Q function - $Q^\policy(\bs,\ba)$, which quantifies the expected return of taking action $\ba$ in state $\bs$ and then following $\policy$:
	\begin{align}
	...
	\end{multline}
	Function approximation under this non-linear operator can cause systematic overestimation, and several attempts exist to overcome its difficulties \cite{Hasselt2015,Mnih2013,Lillicrap2015,Gu2016,Wang2015}.
	...
	An excellent overview of the state of the art MBRL methods is provided in \cite{Wang2019}. Among suitable methods are those based on Gaussian processes and Bayesian neural networks with back-propagation through the dynamics model \cite{Gal2016,6654139}, or those using highly efficient sample-based methods such as the cross-entropy method \cite{Chua2018,Wang2019a,Boer2005}. We focus on model-based data generation.\\
	...
	\end{align}
	Generally, DYNA-style algorithms \cite{Sutton1991} denote methods where an MFRL algorithm is trained on purely synthetic data from an approximate dynamics model $\hat \transitions$, or a mixture of synthetic and real data. We use only synthetic data to reduce the interaction with the real environment to a minimum.\\
	...
	In the following, the most important aspects of a successful application of MBRL are discussed. To our knowledge, this is the first successful application of uncertainty-aware MBRL to an accelerator problem, which, despite its significant potential, demands careful algorithmic design. We made several design decisions that we believe are beneficial for problems typically encountered in accelerator operation.\\
	...
	The primary objective was to evaluate the newly implemented algorithms on a real system and to assess their operational feasibility. 
	...
	\Cref{tab:overview_algorithms} shows a comparison of the discussed algorithms. Details are provided in the appendix. All algorithms are suitable for control tasks as common in accelerator operation. Their strength lies in the high sample efficiency, but they differ in representational power and noise robustness. The \emph{NAF2},
	\emph{ME-TRPO} and \emph{AE-DYNA} are tested and compared on the real environment below. The \emph{NAF2} algorithm, the representative for highly sample-efficient MFRL algorithms, is discussed first.
	\begin{table*}[t]%The best place to locate the table environment is directly after its first reference in text
		\caption{\label{tab:overview_algorithms}%
			...
			\begin{tabular}{lcccc}
				...
				\textrm{Algorithm}&
				\textrm{Class}&
				\textrm{Noise robustness}&
				\textrm{Represent. power}&
				\textrm{Sample efficiency}\\
				\hline
	...
	\begin{figure}
		...
		\caption{The convergence of the \emph{NAF2} algorithm on the FERMI FEL using a single network (dashed) and a double network (solid). The running average of the per-step reward is shown, illustrating faster convergence of the double network variant.}
	...
	\Cref{fig:NAF_training} displays the results, averaged over the two tests \footnote{Considering the training is stochastic with random initial conditions, by chance, the training of the double network variant started above the threshold in both cases.}. Training was conducted over 100 episodes. The number of iterations per episode is plotted, including the cumulative number of steps. In all episodes, during the training, the 95\% intensity reward threshold was surpassed, and therewith they were successfully finished. The evolution of the states is provided in \cref{fig:NAF_evolution_single,fig:NAF_evolution_double} for the single and double network. Learning slows near the domain boundaries where multiple actions map to the same clipped state\footnote{The states are clipped to $[0,1]$.}. The behaviour indicates that the double network converges faster and spends less time at the boundary.\\
	In the verification of 100 episodes, \cref{fig:NAF_verification}, both algorithms show similar performance, while the double network needed less training steps, and reveals a more stable overall performance. Additionally, the convergence of the per-step reward is plotted in \cref{fig:NAF_convergence} against the number of training steps. The double network converges to a high reward level within the shown 700 steps, whereas the single network converges more slowly, reaching a comparable level only after roughly twice as many steps.
	\subsection{MBRL Tests}
	\begin{figure}
		\centering
		\includegraphics*[width=0.5\textwidth]{Figures/AE-DYNA_observables.pdf}
		\caption{The training observables of the \emph{AE-DYNA-SAC} on the FERMI FEL. Details are provided in the text.}
	...
	\begin{figure}
		...
		\caption{The training observables of the \emph{ME-TRPO} on the FERMI FEL. Details are provided in the text.}
	...
	Secondly, the \emph{AE-DYNA-SAC} was tested. As discussed in \cref{ss:critical_design}, in this test, the controller was reset each time the model was re-trained, and consequently the performance drops each time, as shown in \cref{fig:AE-DYNA_observables}.\\
	In contrast to the \emph{ME-TRPO} training, the data batches consisted of 50 data-points with an initial random walk of 200 steps. The number of initial steps was chosen sufficiently large, as insufficient initial data severely degrades convergence, rendering training infeasible on a real machine (discussed in \cref{ss:critical_design}). Each epoch consists of 2500 steps of controller training on the model. The training was stopped after the acquisition of 500 data-points. The verification was executed as in the first test. Again, the success rate of all 50 episodes is 100\%. The number of needed iterations per episode is less than for the ME-TRPO, which can be seen in \cref{fig:AE-DYNA_verification}. The somewhat better performance might be a result of the higher number of data-points (50), but in general, this method exhibited better asymptotic performance than the \emph{ME-TRPO} variant.\\
	...
	In this paper, the applicability of deep reinforcement learning to the optimisation of the FERMI FEL intensity was demonstrated. Two different approaches were tested: model-free and model-based.\\
	Both approaches successfully solved the non-linear, noisy optimisation problem within a feasible number of training steps. The results of the verification are summarised in \cref{tab:overview_verification}. The model-free approach required 800--1000 data points, while the model-based approach required only 450--500. In all experiments, all verification episodes were finished successfully. \emph{NAF2} performs better in terms of episode length while the average reward in the \emph{DYNA}-style algorithms is higher. The experiments were done on different days, hence under slightly different conditions.\\
	Usage of the proposed methods in an operational way is attractive and could in the future replace the current optimisation method, which relies on destructive screen measurements. Hence, online retraining could be performed and valuable time could be saved.\\ 
	The MFRL methods achieved slightly better final performance, but also more samples were collected during training. The MBRL methods had to be stopped due to lack of available experimental time; hence, additional studies regarding long-term performance would be valuable.\\ A significant challenge in applying MBRL methods is the computational time to train the controller on the model. In our tests, data acquisition time was only a fraction of the time needed for controller training. If needed, computational time could be reduced by parallelising the controller training.
	\begin{table*}[t]%The best place to locate the table environment is directly after its first reference in text
		\caption{\label{tab:overview_verification}%
			...
			\begin{tabular}{lrrrr}
				\hline\hline
				&  \centering Data points (counts)&   \centering Episode length (counts)&   \centering Cumulative reward (arb. units)&   \centering Final reward (arb. units)\tabularnewline
				\hline
	...
	\end{table*}
	\section{Conclusions}
	The reinforcement learning methods presented here are well suited for automating setups typical in accelerators. One objective of this work was to provide practical guidance on how to adapt advanced deep reinforcement learning techniques for real-world setups. This was demonstrated on the FERMI FEL intensity optimisation problem.\\
	...
	Complementary, the \emph{NAF2} algorithm presents a good alternative, revealing, as a model-free method, good asymptotic performance. As numerous cases in accelerator control can be captured assuming a quadratic dependence on the actions of the state-action value function, this makes \emph{NAF2} applicable to a broad class of accelerator tuning problems.\\
	To provide the possibility for other laboratories to profit from the stated methods, the code was released in \url{https://github.com/MathPhysSim/FERMI_RL_Paper} and \cite{Hirlaender2020b}.
	
	\section{Acknowledgements}
	We thank Giulio Gaio, who made these experiments possible and helped to set up and restore the FERMI FEL. We are also thankful for the valuable feedback from Alexander Scheinker on the draft of this publication. Finally, we thank the Land Salzburg for their financial support in creating the IDA Lab, which made this work possible.
	
	\appendix
	\section{A Non-linear Standard Control Problem}
	To facilitate reproducibility and comparison with other laboratories, we provide results on a famous classical standard control problem \cite{Furutaa}, the \emph{inverted pendulum}.\\
	It is a non-linear, low-dimensional continuous control problem. In the unsolved variant, no early-termination threshold is imposed on the reward. The episode length, the horizon, is set to 200 steps. In the following, several tests were carried out on the \emph{inverted pendulum} to demonstrate the improvements of the selected algorithms, mainly concerning noise handling. This is of particular importance when dealing with measurements on real systems.\\
	...
	A test adding large artificial Gaussian noise $\epsilon \sim \mathcal{N}(0, \sigma_\epsilon)$ with $\sigma_\epsilon=0.05$ in the normalized observation space on the states is presented in \cref{fig:comparsion_noise}, where the difference of the three methods becomes even more evident. After around 65 episodes the single network without \emph{smoothing} (\emph{No-clipping-no-smoothing}) decreases before reaching the final performance at around 95 episodes, while  \emph{smoothing} prevents this performance drop in the other cases. 
	\begin{figure}
		\centering
	...
	The Bayesian and RL communities are paying increasing attention to new approaches for Bayesian inference. In the `anchored ensembling' technique, a regularization term is added to the loss function, which yields a point estimate of the Bayesian posterior, specifically the maximum a posteriori (MAP) parameter estimate. 
	By employing an ensemble of networks and introducing noise to the regularization term's targets, a distribution of MAP solutions \cite{Gu2007, Chen2011, Bardsley2012, Pearce2018} is obtained, which mimics the true posterior. This approach addresses a key criticism of standard deep ensembles: that randomly initialized networks merely find diverse local optima rather than sampling from the true posterior distribution. The regularization term anchoring the weights to a prior draw provides the theoretical linkage to Bayesian inference that standard ensembles lack.\\
	...
	Finally, \cref{fig:comparsion_NAF_AE-DYNA} demonstrates the sample-efficiency of the \emph{AE-DYNA-SAC} and the \emph{NAF} algorithm on the noisy \emph{inverted pendulum}. \emph{AE-DYNA-SAC} converges below 2000 data-points (without noise even below 800) and the \emph{NAF2} starts to perform equally after 10000 data-points surpassing the -200 reward threshold. One clearly sees the increased sample-efficiency on this problem using the \emph{AE-DYNA} in contrast to the \emph{NAF2}.
